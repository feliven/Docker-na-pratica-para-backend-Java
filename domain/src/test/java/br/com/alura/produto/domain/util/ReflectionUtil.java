            ...
            for (var field : fields) {
                ...
                if (stream(exceto).toList().contains(field.getName()))
                    assertThat(actualFieldValue)
                            .as(format("Expected that [actual.%s] was not equal to [esperado.%s], but it was.",
                                    field.getName(), expectedField.getName()))
                            .isNotEqualTo(expectedFieldValue);
                else
                    assertThat(actualFieldValue)
                            .as(format("Expected that [actual.%s] was equal to [esperado.%s], but it wasn't.",
                                    field.getName(), expectedField.getName()))
        ...
        try {
            ...
            for (var field : fields) {
                ...
                if (!stream(exceto).toList().contains(field.getName())) {
                    if (actualFieldValue instanceof Collection<?> collection)
                        assertThat(collection)
                                .as(format("Expected that [actual.%s] was null or empty, but it wasn't.",
                                        field.getName()))
                                .isNullOrEmpty();
                    else
                        assertThat(actualFieldValue)
